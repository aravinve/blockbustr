  ...
  if ((username === user.username) & (password === user.password)) {
    ...
    cursor.forEach(function (suspiciousRecord) {
      ...
      if (username === suspiciousRecord.username) {
        res.json({
          message:
            'Please contact IT helpdesk for more information, sorry for any inconvenience caused',
          violation: true,
        });
      }
    });
...
route.post('/getcomments', async (req, res) => {
  ...
  postedcomments.forEach(function (doc) {
    if (
      doc.comment.match(/<.+?>/gi) ||
      doc.comment.match(/script/gi) ||
      doc.comment.match(/javascript/gi)
    ) {
      if (
        doc.comment.match(/img/gi) ||
        doc.comment.match(/escape/gi) ||
        doc.comment.match(/alert/gi) ||
        doc.comment.match(/echo/gi) ||
        doc.comment.match(/eval/gi)
      ) {
        if (
          doc.comment.match(/;/gi) ||
          doc.comment.match(/([on]\w+\s*[=]\s*\w+)/gi) ||
          doc.comment.match(/:/gi)
        ) {
        } else {
