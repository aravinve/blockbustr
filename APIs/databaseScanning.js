...
route.post('/scancomments', async (req, res) => {
  ...
  postedcomments.forEach(async function(doc){
		...
		if (suspiciousDoc.match(/(<.+?>)/ig) || suspiciousDoc.match(/script/ig) || suspiciousDoc.match(/javascript/ig)){
			...
			//second level
			if(suspiciousDoc.match(/img/ig) || suspiciousDoc.match(/escape/ig) || suspiciousDoc.match(/alert/ig) || suspiciousDoc.match(/echo/ig) || suspiciousDoc.match(/eval/ig)){
				...
				//third level
				if (suspiciousDoc.match(/;/ig) || suspiciousDoc.match(/([on]\w+\s*[=]\s*\w+)/ig) || suspiciousDoc.match(/:/ig) || suspiciousDoc.match(/((.+?))/ig)){
					
					riskScore += 1;
					sanitizedDoc = sanitizedDoc.replace(/;/ig,"&sc");
					sanitizedDoc = sanitizedDoc.replace(/:/ig,"&c");
					let update = await Comment.updateOne({_id:doc.id},{$set: {comment:sanitizedDoc}});
